...
import * as Ext from './unit-ext.mjs'
...
const CurrentSectionId = Context.Sections.unitHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Unit'
const TitleWhenView = 'View Unit'
const TitleWhenEdit = 'Edit Unit'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('unitHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('unitHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('unitHeaderEdit-btn_new', 'unitHeader-new')
const btn_del = new $fgta5.ActionButton('unitHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('unitHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('unitHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('unitHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('unitHeader-btn_recordstatus')
const btn_logs = document.getElementById('unitHeader-btn_logs')
const btn_about = document.getElementById('unitHeader-btn_about')

const frm = new $fgta5.Form('unitHeaderEdit-frm');
const obj_unit_id = frm.Inputs['unitHeaderEdit-obj_unit_id']
const obj_unit_isdisabled = frm.Inputs['unitHeaderEdit-obj_unit_isdisabled']
const obj_unit_name = frm.Inputs['unitHeaderEdit-obj_unit_name']
const obj_unit_descr = frm.Inputs['unitHeaderEdit-obj_unit_descr']
const obj_struct_id = frm.Inputs['unitHeaderEdit-obj_struct_id']	
...
export async function init(self, args) {
	console.log('initializing unitHeaderEdit ...')
	...
	// export async function unitHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'unitHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function unitHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'unitHeaderEdit_isEditDisabled'
		...
		// export async function unitHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'unitHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.unitHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function unitHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'unitHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function unitHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'unitHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const unitHeaderList = self.Modules.unitHeaderList
	const listsecid = unitHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'unitHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function unitHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.unitHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'unitHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function unitHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'unitHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'unitHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function unitHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.unitHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.unitHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.unitHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.unitHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.unitHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'unitHeaderEdit_addRecordInfo'
