...
const CurrentSectionId = Context.Sections.partnertypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('partnertypeHeaderList-tbl')
const pnl_search = document.getElementById('partnertypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('partnertypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing partnertypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('partnertypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('partnertypeHeaderList-btn_gridload')
		...
		// partnertypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const partnertypeHeaderEdit = self.Modules.partnertypeHeaderEdit
		if (tbl.CurrentRow!=null) {
			partnertypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const partnertypeHeaderEdit = self.Modules.partnertypeHeaderEdit
	partnertypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await partnertypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, partnertypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const partnertypeHeaderEdit = self.Modules.partnertypeHeaderEdit
	partnertypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading partnertypeHeader data')
	console.log(params)
