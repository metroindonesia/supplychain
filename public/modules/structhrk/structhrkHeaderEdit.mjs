...
import * as Ext from './structhrk-ext.mjs'
...
const CurrentSectionId = Context.Sections.structhrkHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Structure Hierarcy'
const TitleWhenView = 'View Structure Hierarcy'
const TitleWhenEdit = 'Edit Structure Hierarcy'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('structhrkHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('structhrkHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('structhrkHeaderEdit-btn_new', 'structhrkHeader-new')
const btn_del = new $fgta5.ActionButton('structhrkHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('structhrkHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('structhrkHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('structhrkHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('structhrkHeader-btn_recordstatus')
const btn_logs = document.getElementById('structhrkHeader-btn_logs')
const btn_about = document.getElementById('structhrkHeader-btn_about')

const frm = new $fgta5.Form('structhrkHeaderEdit-frm');
const obj_structhrk_id = frm.Inputs['structhrkHeaderEdit-obj_structhrk_id']
const obj_structhrk_name = frm.Inputs['structhrkHeaderEdit-obj_structhrk_name']
const obj_structhrk_level = frm.Inputs['structhrkHeaderEdit-obj_structhrk_level']	
...
export async function init(self, args) {
	console.log('initializing structhrkHeaderEdit ...')
	...
	// export async function structhrkHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'structhrkHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function structhrkHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'structhrkHeaderEdit_isEditDisabled'
		...
		// export async function structhrkHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'structhrkHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.structhrkHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function structhrkHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'structhrkHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function structhrkHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'structhrkHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const structhrkHeaderList = self.Modules.structhrkHeaderList
	const listsecid = structhrkHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'structhrkHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function structhrkHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.structhrkHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'structhrkHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function structhrkHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'structhrkHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'structhrkHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function structhrkHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.structhrkHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.structhrkHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.structhrkHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.structhrkHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.structhrkHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'structhrkHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Structure Hierarcy'
