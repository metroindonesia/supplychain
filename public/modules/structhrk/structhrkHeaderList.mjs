...
const CurrentSectionId = Context.Sections.structhrkHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('structhrkHeaderList-tbl')
const pnl_search = document.getElementById('structhrkHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('structhrkHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing structhrkHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('structhrkHeaderList-txt_searchtext')
		const btn_load = document.getElementById('structhrkHeaderList-btn_gridload')
		...
		// structhrkHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const structhrkHeaderEdit = self.Modules.structhrkHeaderEdit
		if (tbl.CurrentRow!=null) {
			structhrkHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const structhrkHeaderEdit = self.Modules.structhrkHeaderEdit
	structhrkHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await structhrkHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, structhrkHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const structhrkHeaderEdit = self.Modules.structhrkHeaderEdit
	structhrkHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading structhrkHeader data')
	console.log(params)
