...
const CurrentSectionId = Context.Sections.currRateList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('currRateList-tbl')

const btn_addrow = document.getElementById('currRateList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('currRateList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.currHeaderEdit
	...
	// export function currRateList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'currRateList_addTableEvents'
	...
	// Extend list detil
	// export function currRateList_init(self) {}
	const fn_name = 'currRateList_init'
	const fn_currRateList_init = Extender[fn_name]
	if (typeof fn_currRateList_init === 'function') {
		fn_currRateList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.currRateList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function currRateList_openList(self, headerForm) {}
	const fn_name = 'currRateList_openList'
	const fn_currRateList_openList = Extender[fn_name]
	if (typeof fn_currRateList_openList === 'function') {
		fn_currRateList_openList(self, headerForm)
	}
...
async function openRow(self, tr) {
	...
	const currRateEdit = self.Modules.currRateEdit
	currRateEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await currRateEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, currRateEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/rate-list`
	const evt = { url, limit }

	// export async function currRateList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'currRateList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/rate-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function currRateList_rowsDeleted(self, data) {}
			const fn_name = 'currRateList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function currRateList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'currRateList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
