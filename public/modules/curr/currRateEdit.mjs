...
const TitleWhenEdit = 'Edit Rate'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('currRateEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('currRateEdit-btn_save')
const btn_new = new $fgta5.ActionButton('currRateEdit-btn_new', 'currRate-addrow')
const btn_del = new $fgta5.ActionButton('currRateEdit-btn_delete', 'currRate-delrow')
const btn_reset = new $fgta5.ActionButton('currRateEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('currRateEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('currRateEdit-btn_next')

const btn_recordstatus = document.getElementById('currRate-btn_recordstatus')
const btn_logs = document.getElementById('currRate-btn_logs')

const frm = new $fgta5.Form('currRateEdit-frm');
const obj_currrate_id = frm.Inputs['currRateEdit-obj_currrate_id']
const obj_currrate_date = frm.Inputs['currRateEdit-obj_currrate_date']
const obj_currrate_value = frm.Inputs['currRateEdit-obj_currrate_value']
const obj_curr_id = frm.Inputs['currRateEdit-obj_curr_id']	
...
export async function init(self, args) {
	...
	CurrentState.getHeaderForm = () => {
		const currHeaderEdit = self.Modules.currHeaderEdit
		const frmHeader = currHeaderEdit.getHeaderForm()
		return frmHeader
	}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'currRateEdit_isEditDisabled'
		...
		// export function currRateEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'currRateEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function currRateEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'currRateEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function currRateEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'currRateEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/rate-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/rate-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/rate-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/rate-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.currRateList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function currRateEdit_formUnlocked(self, frm) {}
	const fn_name = 'currRateEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const currRateList = self.Modules.currRateList
	const listsecid = currRateList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(currRateList.Section)
	...
	try {
		...
		const curr_id = header_pk.value

		// inisiasi data baru
		const datainit = {
			curr_id,
			currrate_date: new Date(),
			currrate_value: 0,
		}
		...
		// export async function currRateEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'currRateEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.currRateList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function currRateEdit_autofill(self, frm) {}
	const fn_autofill_name = 'currRateEdit_autofill'
	...
	// Extender Saving
	// export async function currRateEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'currRateEdit_dataSaving'
	...
	try {
		...
		if (isNewData) {
			...
			self.Modules.currRateList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.currRateList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		// Extender Deleting
		// export async function currRateEdit_dataDeleting(self, id, args) {}
		const args = { cancelDelete: false }
		const fn_datadeleting_name = 'currRateEdit_dataDeleting'
		...
		// Extender Delete
		// export async function currRateEdit_dataDeleted(self, data) {}
		const fn_datadeleted_name = 'currRateEdit_dataDeleted'
		...
		self.Modules.currRateList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.currRateList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.currRateList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender currRateEdit_addRecordInfo
			// export async function currRateEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'currRateEdit_addRecordInfo'
