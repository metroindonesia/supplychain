...
const CurrentSectionId = Context.Sections.currHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Currency'
const TitleWhenView = 'View Currency'
const TitleWhenEdit = 'Edit Currency'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('currHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('currHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('currHeaderEdit-btn_new', 'currHeader-new')
const btn_del = new $fgta5.ActionButton('currHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('currHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('currHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('currHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('currHeader-btn_recordstatus')
const btn_logs = document.getElementById('currHeader-btn_logs')
const btn_about = document.getElementById('currHeader-btn_about')

const frm = new $fgta5.Form('currHeaderEdit-frm');
const obj_curr_id = frm.Inputs['currHeaderEdit-obj_curr_id']
const obj_curr_name = frm.Inputs['currHeaderEdit-obj_curr_name']
const obj_curr_descr = frm.Inputs['currHeaderEdit-obj_curr_descr']
const obj_curr_code = frm.Inputs['currHeaderEdit-obj_curr_code']	
...
export async function init(self, args) {
	console.log('initializing currHeaderEdit ...')
	...
	// export async function currHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'currHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function currHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'currHeaderEdit_isEditDisabled'
		...
		// export async function currHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'currHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.currHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function currHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'currHeaderEdit_formLocked'
	...
	// trigger lock event di rate
	self.Modules.currRateList.headerLocked(self)
	self.Modules.currRateEdit.headerLocked(self)
...
async function  frm_unlocked(self, evt) {
	...
	// export function currHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'currHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
	...
	// trigger unlock event di rate
	self.Modules.currRateList.headerUnlocked(self)
	self.Modules.currRateEdit.headerUnlocked(self)	
...
async function btn_new_click(self, evt) {
	...
	const currHeaderList = self.Modules.currHeaderList
	const listsecid = currHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'currHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function currHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.currHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'currHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function currHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'currHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'currHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function currHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.currHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.currHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.currHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.currHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.currHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'currHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Currency'
