...
const obj_delegate_user_id = frm.Inputs['authHeaderEdit-obj_delegate_user_id']
const obj_delegate_start = frm.Inputs['authHeaderEdit-obj_delegate_start']
const obj_delegate_end = frm.Inputs['authHeaderEdit-obj_delegate_end']	
...
export async function init(self, args) {
	...
	// Combobox: obj_delegate_user_id
	obj_delegate_user_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_delegate_user_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_delegate_user_id_selecting(self, obj_delegate_user_id, frm, evt) {}
			fn_selecting(self, obj_delegate_user_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			...
			// buat function di extender:
			// export function obj_delegate_user_id_selecting_criteria(self, obj_delegate_user_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_delegate_user_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_delegate_user_id, frm, criteria, sort, evt)
			}
...
async function btn_new_click(self, evt) {
	...
	try {
		...
		// inisiasi data baru
		const datainit = {
			delegate_start: new Date(),
			delegate_end: new Date(),
		}
