...
const CurrentSectionId = Context.Sections.partnerContactList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('partnerContactList-tbl')

const btn_addrow = document.getElementById('partnerContactList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('partnerContactList-btn_delrow')
...
export async function init(self, args) {
	...
	// Extend list detil
	// export function partnerContactList_init(self) {}
	const fn_name = 'partnerContactList_init'
	const fn_partnerContactList_init = Extender[fn_name]
	if (typeof fn_partnerContactList_init === 'function') {
		fn_partnerContactList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.partnerContactList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function partnerContactList_openList(self, headerForm) {}
	const fn_name = 'partnerContactList_openList'
	const fn_partnerContactList_openList = Extender[fn_name]
	if (typeof fn_partnerContactList_openList === 'function') {
		fn_partnerContactList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			partnerContactEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const partnerContactEdit = self.Modules.partnerContactEdit
	partnerContactEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await partnerContactEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, partnerContactEdit)
}
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/contact-delete-rows`
	try {
