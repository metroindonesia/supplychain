...
const CurrentSectionId = Context.Sections.partnerHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('partnerHeaderList-tbl')
const pnl_search = document.getElementById('partnerHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('partnerHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing partnerHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('partnerHeaderList-txt_searchtext')
		const btn_load = document.getElementById('partnerHeaderList-btn_gridload')
		...
		// partnerHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			partnerHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const partnerHeaderEdit = self.Modules.partnerHeaderEdit
	partnerHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await partnerHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, partnerHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const partnerHeaderEdit = self.Modules.partnerHeaderEdit
	partnerHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading partnerHeader data')
	console.log(params)
