...
const CurrentSectionId = Context.Sections.partnerBankList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('partnerBankList-tbl')

const btn_addrow = document.getElementById('partnerBankList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('partnerBankList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.partnerHeaderEdit
	...
	// export function partnerBankList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'partnerBankList_addTableEvents'
	...
	// Extend list detil
	// export function partnerBankList_init(self) {}
	const fn_name = 'partnerBankList_init'
	const fn_partnerBankList_init = Extender[fn_name]
	if (typeof fn_partnerBankList_init === 'function') {
		fn_partnerBankList_init(self)
	}
...
function setDefaultHeadTitle(self, headerForm) {
	...
	for (let el of detilTitleElements) {
		el.innerHTML = data.partner_name
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.partnerBankList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function partnerBankList_openList(self, headerForm) {}
	const fn_name = 'partnerBankList_openList'
	const fn_partnerBankList_openList = Extender[fn_name]
	if (typeof fn_partnerBankList_openList === 'function') {
		fn_partnerBankList_openList(self, headerForm)
	}
	...
	const partnerBankEdit = self.Modules.partnerBankEdit
	const btn_addrow = partnerBankEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			partnerBankEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const partnerBankEdit = self.Modules.partnerBankEdit
	partnerBankEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await partnerBankEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, partnerBankEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/bank-list`
	const evt = { url, limit }

	// export async function partnerBankList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'partnerBankList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/bank-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function partnerBankList_rowsDeleted(self, data) {}
			const fn_name = 'partnerBankList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function partnerBankList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'partnerBankList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
