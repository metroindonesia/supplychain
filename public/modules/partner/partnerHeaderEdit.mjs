...
const CurrentSectionId = Context.Sections.partnerHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Partner'
const TitleWhenView = 'View Partner'
const TitleWhenEdit = 'Edit Partner'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('partnerHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('partnerHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('partnerHeaderEdit-btn_new', 'partnerHeader-new')
const btn_del = new $fgta5.ActionButton('partnerHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('partnerHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('partnerHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('partnerHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('partnerHeader-btn_recordstatus')
const btn_logs = document.getElementById('partnerHeader-btn_logs')
const btn_about = document.getElementById('partnerHeader-btn_about')

const frm = new $fgta5.Form('partnerHeaderEdit-frm');
const obj_partner_id = frm.Inputs['partnerHeaderEdit-obj_partner_id']
const obj_partner_isdisabled = frm.Inputs['partnerHeaderEdit-obj_partner_isdisabled']
const obj_partner_isemployee = frm.Inputs['partnerHeaderEdit-obj_partner_isemployee']
const obj_partner_name = frm.Inputs['partnerHeaderEdit-obj_partner_name']
const obj_partner_legaltitle = frm.Inputs['partnerHeaderEdit-obj_partner_legaltitle']
const obj_partnertype_id = frm.Inputs['partnerHeaderEdit-obj_partnertype_id']
const obj_employee_nip = frm.Inputs['partnerHeaderEdit-obj_employee_nip']
const obj_partner_address = frm.Inputs['partnerHeaderEdit-obj_partner_address']
const obj_partner_city = frm.Inputs['partnerHeaderEdit-obj_partner_city']
const obj_partner_email = frm.Inputs['partnerHeaderEdit-obj_partner_email']
const obj_partner_npwp = frm.Inputs['partnerHeaderEdit-obj_partner_npwp']
const obj_partner_isnonpkp = frm.Inputs['partnerHeaderEdit-obj_partner_isnonpkp']	
...
export async function init(self, args) {
	console.log('initializing partnerHeaderEdit ...')
	...
	// export async function partnerHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'partnerHeaderEdit_init'
	...
	// Combobox: obj_partnertype_id
	obj_partnertype_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_partnertype_id_selected'
		...
		if (typeof fn_selected === 'function') {
			...
			fn_selected(self, obj_partnertype_id, frm, evt)
		} else {	
			console.warn('Extender.obj_partnertype_id_selected is not implemented')
		}		
	})
	
	obj_partnertype_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_partnertype_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_partnertype_id_selecting(self, obj_partnertype_id, frm, evt) {}
			fn_selecting(self, obj_partnertype_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'partnertype/header-list'
			const sort = {}
			const criteria = {
			...
			// export function obj_partnertype_id_selecting_criteria(self, obj_partnertype_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_partnertype_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_partnertype_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.partnertype_id, row.partnertype_name, row)
				}
	...
	// Checkbox: obj_partner_isnonpkp
	obj_partner_isnonpkp.addEventListener('changed', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_changed_name = 'obj_partner_isnonpkp_changed'
		...
		if (typeof fn_changed === 'function') {
			...
			fn_changed(self, obj_partner_isnonpkp, frm, evt)
		} else {	
			console.warn('Extender.obj_partner_isnonpkp_changed is not implemented')
		}		
	})
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function partnerHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'partnerHeaderEdit_isEditDisabled'
		...
		const fn_formopened_name = 'partnerHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.partnerHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function partnerHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'partnerHeaderEdit_formLocked'
	...
	// trigger lock event di bank
	self.Modules.partnerBankList.headerLocked(self)
	self.Modules.partnerBankEdit.headerLocked(self)
...
async function  frm_unlocked(self, evt) {
	...
	// export function partnerHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'partnerHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
	...
	// trigger unlock event di bank
	self.Modules.partnerBankList.headerUnlocked(self)
	self.Modules.partnerBankEdit.headerUnlocked(self)	
...
async function btn_new_click(self, evt) {
	...
	const partnerHeaderList = self.Modules.partnerHeaderList
	const listsecid = partnerHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'partnerHeaderEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.partnerHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'partnerHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function partnerHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'partnerHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'partnerHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function partnerHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.partnerHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.partnerHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.partnerHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.partnerHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.partnerHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'partnerHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Partner'
