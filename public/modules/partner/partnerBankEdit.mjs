...
const Extender = Ext.extenderBank ?? Ext


const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.partnerBankEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Bank'
const TitleWhenView = 'View Bank'
const TitleWhenEdit = 'Edit Bank'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('partnerBankEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('partnerBankEdit-btn_save')
const btn_new = new $fgta5.ActionButton('partnerBankEdit-btn_new', 'partnerBank-addrow')
const btn_del = new $fgta5.ActionButton('partnerBankEdit-btn_delete', 'partnerBank-delrow')
const btn_reset = new $fgta5.ActionButton('partnerBankEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('partnerBankEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('partnerBankEdit-btn_next')

const btn_recordstatus = document.getElementById('partnerBank-btn_recordstatus')
const btn_logs = document.getElementById('partnerBank-btn_logs')

const frm = new $fgta5.Form('partnerBankEdit-frm');
const obj_partnerbank_id = frm.Inputs['partnerBankEdit-obj_partnerbank_id']
const obj_partnerbank_account = frm.Inputs['partnerBankEdit-obj_partnerbank_account']
const obj_partnerbank_isdisabled = frm.Inputs['partnerBankEdit-obj_partnerbank_isdisabled']
const obj_partnerbank_accountname = frm.Inputs['partnerBankEdit-obj_partnerbank_accountname']
const obj_partnerbank_bankname = frm.Inputs['partnerBankEdit-obj_partnerbank_bankname']
const obj_partnerbank_name = frm.Inputs['partnerBankEdit-obj_partnerbank_name']
const obj_partner_id = frm.Inputs['partnerBankEdit-obj_partner_id']	
...
export async function init(self, args) {
	...
	CurrentState.getHeaderForm = () => {
		const partnerHeaderEdit = self.Modules.partnerHeaderEdit
		const frmHeader = partnerHeaderEdit.getHeaderForm()
		return frmHeader
	}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'partnerBankEdit_isEditDisabled'
		...
		// export function partnerBankEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'partnerBankEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function partnerBankEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'partnerBankEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function partnerBankEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'partnerBankEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/bank-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/bank-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/bank-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/bank-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.partnerBankList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function partnerBankEdit_formUnlocked(self, frm) {}
	const fn_name = 'partnerBankEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const partnerBankList = self.Modules.partnerBankList
	const listsecid = partnerBankList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(partnerBankList.Section)
	...
	try {
		...
		const partner_id = header_pk.value
		...
		// export async function partnerBankEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'partnerBankEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.partnerBankList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function partnerBankEdit_autofill(self, frm) {}
	const fn_autofill_name = 'partnerBankEdit_autofill'
	...
	// Extender Saving
	// export async function partnerBankEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'partnerBankEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'partnerBankEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function partnerBankEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.partnerBankList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.partnerBankList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		// Extender Deleting
		// export async function partnerBankEdit_dataDeleting(self, id, args) {}
		const args = { cancelDelete: false }
		const fn_datadeleting_name = 'partnerBankEdit_dataDeleting'
		...
		// Extender Delete
		// export async function partnerBankEdit_dataDeleted(self, data) {}
		const fn_datadeleted_name = 'partnerBankEdit_dataDeleted'
		...
		self.Modules.partnerBankList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.partnerBankList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.partnerBankList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender partnerBankEdit_addRecordInfo
			// export async function partnerBankEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'partnerBankEdit_addRecordInfo'
