...
const TitleWhenEdit = 'Edit Contact'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('partnerContactEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('partnerContactEdit-btn_save')
const btn_new = new $fgta5.ActionButton('partnerContactEdit-btn_new', 'partnerContact-addrow')
const btn_del = new $fgta5.ActionButton('partnerContactEdit-btn_delete', 'partnerContact-delrow')
const btn_reset = new $fgta5.ActionButton('partnerContactEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('partnerContactEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('partnerContactEdit-btn_next')

const btn_recordstatus = document.getElementById('partnerContact-btn_recordstatus')
const btn_logs = document.getElementById('partnerContact-btn_logs')

const frm = new $fgta5.Form('partnerContactEdit-frm');
const obj_partnercontact_id = frm.Inputs['partnerContactEdit-obj_partnercontact_id']
const obj_partnercontact_name = frm.Inputs['partnerContactEdit-obj_partnercontact_name']
const obj_partnercontact_isdisabled = frm.Inputs['partnerContactEdit-obj_partnercontact_isdisabled']
const obj_partnercontact_position = frm.Inputs['partnerContactEdit-obj_partnercontact_position']
const obj_partnercontact_hp = frm.Inputs['partnerContactEdit-obj_partnercontact_hp']
const obj_partnercontact_email = frm.Inputs['partnerContactEdit-obj_partnercontact_email']
const obj_partner_id = frm.Inputs['partnerContactEdit-obj_partner_id']	
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'partnerContactEdit_isEditDisabled'
		...
		// export function partnerContactEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'partnerContactEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function partnerContactEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'partnerContactEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function partnerContactEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'partnerContactEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/contact-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/contact-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/contact-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/contact-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.partnerContactList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function partnerContactEdit_formUnlocked(self, frm) {}
	const fn_name = 'partnerContactEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const partnerContactList = self.Modules.partnerContactList
	const listsecid = partnerContactList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(partnerContactList.Section)
	...
	try {
		...
		// export async function partnerContactEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'partnerContactEdit_newData'
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function partnerContactEdit_autofill(self, frm) {}
	const fn_autofill_name = 'partnerContactEdit_autofill'
	...
	// Extender Saving
	// export async function partnerContactEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'partnerContactEdit_dataSaving'
	...
	try {
		...
		if (isNewData) {
			...
			self.Modules.partnerContactList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.partnerContactList.updateCurrentRow(self, data)
		}
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.partnerContactList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.partnerContactList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender partnerContactEdit_addRecordInfo
			// export async function partnerContactEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'partnerContactEdit_addRecordInfo'
