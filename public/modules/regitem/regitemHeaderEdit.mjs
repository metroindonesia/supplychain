...
import * as Ext from './regitem-ext.mjs'
...
const CurrentSectionId = Context.Sections.regitemHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Register Item'
const TitleWhenView = 'View Register Item'
const TitleWhenEdit = 'Edit Register Item'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('regitemHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('regitemHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('regitemHeaderEdit-btn_new', 'regitemHeader-new')
const btn_del = new $fgta5.ActionButton('regitemHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('regitemHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('regitemHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('regitemHeaderEdit-btn_next')

const btn_actionCommit = new $fgta5.ActionButton('regitemHeaderEdit-btn_actionCommit')
const btn_actionUncommit = new $fgta5.ActionButton('regitemHeaderEdit-btn_actionUncommit')
const btn_actionGenerate = new $fgta5.ActionButton('regitemHeaderEdit-btn_actionGenerate')

const btn_recordstatus = document.getElementById('regitemHeader-btn_recordstatus')
const btn_logs = document.getElementById('regitemHeader-btn_logs')
const btn_about = document.getElementById('regitemHeader-btn_about')

const frm = new $fgta5.Form('regitemHeaderEdit-frm');
const obj_regitem_id = frm.Inputs['regitemHeaderEdit-obj_regitem_id']
const obj_regitemtype_id = frm.Inputs['regitemHeaderEdit-obj_regitemtype_id']	
...
export async function init(self, args) {
	console.log('initializing regitemHeaderEdit ...')
	...
	CurrentState.Actions = {
		...
		generate: btn_actionGenerate,	
	}
	
	// export async function regitemHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'regitemHeaderEdit_init'
	...
	// Combobox: obj_regitemtype_id
	obj_regitemtype_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_regitemtype_id_selected'
		const fn_selected = Extender[fn_selected_name]
		if (typeof fn_selected === 'function') {
			// create function di Extender:
			// export async function obj_regitemtype_id_selected(self, obj_regitemtype_id, frm, evt) {}
			fn_selected(self, obj_regitemtype_id, frm, evt)
		} else {	
			console.warn('Extender.obj_regitemtype_id_selected is not implemented')
		}		
	})
	
	obj_regitemtype_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_regitemtype_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_regitemtype_id_selecting(self, obj_regitemtype_id, frm, evt) {}
			fn_selecting(self, obj_regitemtype_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'regitemtype/header-list'
			const sort = {}
			const criteria = {
			...
			// export function obj_regitemtype_id_selecting_criteria(self, obj_regitemtype_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_regitemtype_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_regitemtype_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.regitemtype_id, row.regitemtype_name, row)
				}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function regitemHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'regitemHeaderEdit_isEditDisabled'
		...
		// export async function regitemHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'regitemHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.regitemHeaderList
...
async function  frm_locked(self, evt) {
	...
	btn_actionGenerate.disabled = false
	
	// Extender untuk event locked
	// export function regitemHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'regitemHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	btn_actionGenerate.disabled = true

	// Extender untuk event Unlocked
	// export function regitemHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'regitemHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const regitemHeaderList = self.Modules.regitemHeaderList
	const listsecid = regitemHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'regitemHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function regitemHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.regitemHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'regitemHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function regitemHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'regitemHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'regitemHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function regitemHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.regitemHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.regitemHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.regitemHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.regitemHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.regitemHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'regitemHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Register Item'
		...
		if ( document.getElementById('fAbout-section-fdescr') == null) {
			...
			divDescr.innerHTML = 'registrasi barang baru'
