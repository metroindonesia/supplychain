...
const CurrentSectionId = Context.Sections.regitemHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('regitemHeaderList-tbl')
const pnl_search = document.getElementById('regitemHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('regitemHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing regitemHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('regitemHeaderList-txt_searchtext')
		const btn_load = document.getElementById('regitemHeaderList-btn_gridload')
		...
		// regitemHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const regitemHeaderEdit = self.Modules.regitemHeaderEdit
		if (tbl.CurrentRow!=null) {
			regitemHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const regitemHeaderEdit = self.Modules.regitemHeaderEdit
	regitemHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await regitemHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, regitemHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const regitemHeaderEdit = self.Modules.regitemHeaderEdit
	regitemHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading regitemHeader data')
	console.log(params)
