...
const CurrentSectionId = Context.Sections.structMemberList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('structMemberList-tbl')

const btn_addrow = document.getElementById('structMemberList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('structMemberList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.structHeaderEdit
	...
	// export function structMemberList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'structMemberList_addTableEvents'
	...
	// Extend list detil
	// export function structMemberList_init(self) {}
	const fn_name = 'structMemberList_init'
	const fn_structMemberList_init = Extender[fn_name]
	if (typeof fn_structMemberList_init === 'function') {
		fn_structMemberList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.structMemberList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function structMemberList_openList(self, headerForm) {}
	const fn_name = 'structMemberList_openList'
	const fn_structMemberList_openList = Extender[fn_name]
	if (typeof fn_structMemberList_openList === 'function') {
		fn_structMemberList_openList(self, headerForm)
	}
	...
	const structMemberEdit = self.Modules.structMemberEdit
	const btn_addrow = structMemberEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			structMemberEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const structMemberEdit = self.Modules.structMemberEdit
	structMemberEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await structMemberEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, structMemberEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/member-list`
	const evt = { url, limit }

	// export async function structMemberList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'structMemberList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/member-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function structMemberList_rowsDeleted(self, data) {}
			const fn_name = 'structMemberList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function structMemberList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'structMemberList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
