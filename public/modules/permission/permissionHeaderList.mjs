...
const CurrentSectionId = Context.Sections.permissionHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('permissionHeaderList-tbl')
const pnl_search = document.getElementById('permissionHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('permissionHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing permissionHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('permissionHeaderList-txt_searchtext')
		const btn_load = document.getElementById('permissionHeaderList-btn_gridload')
		...
		// permissionHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const permissionHeaderEdit = self.Modules.permissionHeaderEdit
		if (tbl.CurrentRow!=null) {
			permissionHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const permissionHeaderEdit = self.Modules.permissionHeaderEdit
	permissionHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await permissionHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, permissionHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const permissionHeaderEdit = self.Modules.permissionHeaderEdit
	permissionHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading permissionHeader data')
	console.log(params)
