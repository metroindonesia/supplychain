...
const CurrentSectionId = Context.Sections.brandHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('brandHeaderList-tbl')
const pnl_search = document.getElementById('brandHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('brandHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing brandHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('brandHeaderList-txt_searchtext')
		const btn_load = document.getElementById('brandHeaderList-btn_gridload')
		...
		// brandHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const brandHeaderEdit = self.Modules.brandHeaderEdit
		if (tbl.CurrentRow!=null) {
			brandHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const brandHeaderEdit = self.Modules.brandHeaderEdit
	brandHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await brandHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, brandHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const brandHeaderEdit = self.Modules.brandHeaderEdit
	brandHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading brandHeader data')
	console.log(params)
