...
import * as Ext from './brand-ext.mjs'
...
const CurrentSectionId = Context.Sections.brandHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New brand'
const TitleWhenView = 'View brand'
const TitleWhenEdit = 'Edit brand'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('brandHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('brandHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('brandHeaderEdit-btn_new', 'brandHeader-new')
const btn_del = new $fgta5.ActionButton('brandHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('brandHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('brandHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('brandHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('brandHeader-btn_recordstatus')
const btn_logs = document.getElementById('brandHeader-btn_logs')
const btn_about = document.getElementById('brandHeader-btn_about')

const frm = new $fgta5.Form('brandHeaderEdit-frm');
const obj_brand_id = frm.Inputs['brandHeaderEdit-obj_brand_id']
const obj_brand_isdisabled = frm.Inputs['brandHeaderEdit-obj_brand_isdisabled']
const obj_brand_name = frm.Inputs['brandHeaderEdit-obj_brand_name']
const obj_brand_descr = frm.Inputs['brandHeaderEdit-obj_brand_descr']
const obj_unit_id = frm.Inputs['brandHeaderEdit-obj_unit_id']
const obj_struct_id = frm.Inputs['brandHeaderEdit-obj_struct_id']	
...
export async function init(self, args) {
	console.log('initializing brandHeaderEdit ...')
	...
	// export async function brandHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'brandHeaderEdit_init'
	...
	obj_unit_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_unit_id_selected'
		const fn_selected = Extender[fn_selected_name]
		if (typeof fn_selected === 'function') {
			// create function di Extender:
			// export async function obj_unit_id_selected(self, obj_unit_id, frm, evt) {}
			fn_selected(self, obj_unit_id, frm, evt)
		} else {	
			console.warn('Extender.obj_unit_id_selected is not implemented')
		}		
	})
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function brandHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'brandHeaderEdit_isEditDisabled'
		...
		// export async function brandHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'brandHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.brandHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function brandHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'brandHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function brandHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'brandHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const brandHeaderList = self.Modules.brandHeaderList
	const listsecid = brandHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'brandHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function brandHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.brandHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'brandHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function brandHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'brandHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'brandHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function brandHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.brandHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.brandHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.brandHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.brandHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.brandHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'brandHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About brand'
		...
		if ( document.getElementById('fAbout-section-fdescr') == null) {
			...
			divDescr.innerHTML = 'daftar brand'
