...
const CurrentSectionId = Context.Sections.itemclassStructList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('itemclassStructList-tbl')

const btn_addrow = document.getElementById('itemclassStructList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('itemclassStructList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.itemclassHeaderEdit
	...
	// export function itemclassStructList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'itemclassStructList_addTableEvents'
	...
	// Extend list detil
	// export function itemclassStructList_init(self) {}
	const fn_name = 'itemclassStructList_init'
	const fn_itemclassStructList_init = Extender[fn_name]
	if (typeof fn_itemclassStructList_init === 'function') {
		fn_itemclassStructList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.itemclassStructList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function itemclassStructList_openList(self, headerForm) {}
	const fn_name = 'itemclassStructList_openList'
	const fn_itemclassStructList_openList = Extender[fn_name]
	if (typeof fn_itemclassStructList_openList === 'function') {
		fn_itemclassStructList_openList(self, headerForm)
	}
	...
	const itemclassStructEdit = self.Modules.itemclassStructEdit
	const btn_addrow = itemclassStructEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			itemclassStructEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const itemclassStructEdit = self.Modules.itemclassStructEdit
	itemclassStructEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await itemclassStructEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, itemclassStructEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/struct-list`
	const evt = { url, limit }

	// export async function itemclassStructList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'itemclassStructList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/struct-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function itemclassStructList_rowsDeleted(self, data) {}
			const fn_name = 'itemclassStructList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function itemclassStructList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'itemclassStructList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
