...
import * as Extender from './extenders/partnertype.apiext.js'

const moduleName = 'partnertype'
const headerSectionName = 'header'
const headerTableName = 'public.partnertype' 
const headerPrimaryKey = 'partnertype_id' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await partnertype_init(this, body) }

	// extender call
	async execute(body) { return await partnertype_execute(this, body) }

	// header
	async headerList(body) { return await partnertype_headerList(this, body) }
	async headerOpen(body) { return await partnertype_headerOpen(this, body) }
	async headerUpdate(body) { return await partnertype_headerUpdate(this, body)}
	async headerCreate(body) { return await partnertype_headerCreate(this, body)}
	async headerDelete(body) { return await partnertype_headerDelete(this, body) }

			
}	

// init module
async function partnertype_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.partnertype_init === 'function') {
			// export async function partnertype_init(self, initialData) {}
			await Extender.partnertype_init(self, initialData)
		}
...
async function partnertype_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, partnertype_log) {}
		return await Extender[fnName](self, db, body, partnertype_log)
	} else {
...
// data logging
async function partnertype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function partnertype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `partnertype_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function partnertype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { partnertype_id: id }
		const searchMap = { partnertype_id: `partnertype_id = \${partnertype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function partnertype_headerCreate(self, body) {
	const { source='partnertype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			partnertype_log(self, body, startTime, tablename, ret.partnertype_id, 'CREATE', logMetadata)
...
async function partnertype_headerUpdate(self, body) {
	const { source='partnertype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['partnertype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			partnertype_log(self, body, startTime, tablename, data.partnertype_id, 'UPDATE')
...
async function partnertype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {partnertype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['partnertype_id'])
			...
			// record log
			partnertype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
