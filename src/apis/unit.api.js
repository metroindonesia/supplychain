...
import * as Extender from './extenders/unit.apiext.js'
...
// init module
async function unit_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.unit_init === 'function') {
			// export async function unit_init(self, initialData) {}
			await Extender.unit_init(self, initialData)
		}
...
async function unit_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `unit_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function unit_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { unit_id: id }
		const searchMap = { unit_id: `unit_id = \${unit_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function unit_headerCreate(self, body) {
	const { source='unit', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			unit_log(self, body, startTime, tablename, ret.unit_id, 'CREATE', logMetadata)
...
async function unit_headerUpdate(self, body) {
	const { source='unit', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['unit_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			unit_log(self, body, startTime, tablename, data.unit_id, 'UPDATE')
...
async function unit_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['unit_id'])
			...
			// record log
			unit_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
