...
import * as Extender from './extenders/structhrk.apiext.js'

const moduleName = 'structhrk'
const headerSectionName = 'header'
const headerTableName = 'public.structhrk' 
const headerPrimaryKey = 'structhrk_id' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await structhrk_init(this, body) }

	// extender call
	async execute(body) { return await structhrk_execute(this, body) }

	// header
	async headerList(body) { return await structhrk_headerList(this, body) }
	async headerOpen(body) { return await structhrk_headerOpen(this, body) }
	async headerUpdate(body) { return await structhrk_headerUpdate(this, body)}
	async headerCreate(body) { return await structhrk_headerCreate(this, body)}
	async headerDelete(body) { return await structhrk_headerDelete(this, body) }

			
}	

// init module
async function structhrk_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.structhrk_init === 'function') {
			// export async function structhrk_init(self, initialData) {}
			await Extender.structhrk_init(self, initialData)
		}
...
async function structhrk_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, structhrk_log) {}
		return await Extender[fnName](self, db, body, structhrk_log)
	} else {
...
// data logging
async function structhrk_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function structhrk_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `structhrk_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function structhrk_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { structhrk_id: id }
		const searchMap = { structhrk_id: `structhrk_id = \${structhrk_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function structhrk_headerCreate(self, body) {
	const { source='structhrk', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			structhrk_log(self, body, startTime, tablename, ret.structhrk_id, 'CREATE', logMetadata)
...
async function structhrk_headerUpdate(self, body) {
	const { source='structhrk', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['structhrk_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			structhrk_log(self, body, startTime, tablename, data.structhrk_id, 'UPDATE')
...
async function structhrk_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {structhrk_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['structhrk_id'])
			...
			// record log
			structhrk_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
