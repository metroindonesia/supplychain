...
import * as Extender from './extenders/permission.apiext.js'

const moduleName = 'permission'
const headerSectionName = 'header'
const headerTableName = 'core.permission' 
const headerPrimaryKey = 'permission_id' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await permission_init(this, body) }

	// extender call
	async execute(body) { return await permission_execute(this, body) }

	// header
	async headerList(body) { return await permission_headerList(this, body) }
	async headerOpen(body) { return await permission_headerOpen(this, body) }
	async headerUpdate(body) { return await permission_headerUpdate(this, body)}
	async headerCreate(body) { return await permission_headerCreate(this, body)}
	async headerDelete(body) { return await permission_headerDelete(this, body) }

			
}	

// init module
async function permission_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.permission_init === 'function') {
			// export async function permission_init(self, initialData) {}
			await Extender.permission_init(self, initialData)
		}
...
async function permission_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, permission_log) {}
		return await Extender[fnName](self, db, body, permission_log)
	} else {
...
// data logging
async function permission_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function permission_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `permission_name ILIKE '%' || \${searchtext} || '%' OR permission_descr ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function permission_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { permission_id: id }
		const searchMap = { permission_id: `permission_id = \${permission_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function permission_headerCreate(self, body) {
	const { source='permission', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const cmd = sqlUtil.createInsertCommand(tablename, data, ['permission_id'])
			...
			// record log
			permission_log(self, body, startTime, tablename, ret.permission_id, 'CREATE', logMetadata)
...
async function permission_headerUpdate(self, body) {
	const { source='permission', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['permission_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			permission_log(self, body, startTime, tablename, data.permission_id, 'UPDATE')
...
async function permission_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {permission_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['permission_id'])
			...
			// record log
			permission_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
